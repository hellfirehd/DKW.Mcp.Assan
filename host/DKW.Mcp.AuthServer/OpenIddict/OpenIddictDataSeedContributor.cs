...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
	...
	{
		if (await _scopeManager.FindByNameAsync("Mcp") == null)
		{
	...
	private async Task CreateApplicationsAsync()
	{
		...
		//Web Client
		var webClientId = configurationSection["Mcp_Web:ClientId"];
		if (!webClientId.IsNullOrWhiteSpace())
		{
			var webClientRootUrl = configurationSection["Mcp_Web:RootUrl"]?.EnsureEndsWith('/')
				?? throw new AbpException("Mcp_Web:RootUrl has not been defined");

			/* Mcp_Web client is only needed if you created a tiered
			...
			await CreateApplicationAsync(
				...
				secret: configurationSection["Mcp_Web:ClientSecret"] ?? "1q2w3e*",
		...
		var consoleAndAngularClientId = configurationSection["Mcp_App:ClientId"];
		if (!consoleAndAngularClientId.IsNullOrWhiteSpace())
		{
			var consoleAndAngularClientRootUrl = configurationSection["Mcp_App:RootUrl"]?.TrimEnd('/');
			await CreateApplicationAsync(
		...
		// Blazor Client
		var blazorClientId = configurationSection["Mcp_Blazor:ClientId"];
		if (!blazorClientId.IsNullOrWhiteSpace())
		{
			var blazorRootUrl = configurationSection["Mcp_Blazor:RootUrl"]?.TrimEnd('/');
		...
		// Swagger Client
		var swaggerClientId = configurationSection["Mcp_Swagger:ClientId"];
		if (!swaggerClientId.IsNullOrWhiteSpace())
		{
			var swaggerRootUrl = configurationSection["Mcp_Swagger:RootUrl"]?.TrimEnd('/');
